# OnPurpose Marketplace - Complete Codebase

## Project Overview
OnPurpose — Book People, Not Places. A human services marketplace server.
...
```json
{
  "name": "onpurpose",
  "version": "2.0.0",
  "description": "OnPurpose — Book People, Not Places. A human services marketplace.",
  "main": "server.js",
  "scripts": {
    "start": "npx sequelize-cli db:migrate && node server.js",
    "dev": "nodemon server.js",
    "seed": "node seed.js",
    "generate:secrets": "node -e \"const c=require('crypto');console.log('JWT_SECRET='+c.randomBytes(64).toString('hex'));console.log('REFRESH_TOKEN_SECRET='+c.randomBytes(64).toString('hex'));console.log('BCRYPT_PEPPER='+c.randomBytes(32).toString('hex'));console.log('QR_SECRET='+c.randomBytes(32).toString('hex'));\""
  },
  "engines": {
    "node": ">=18.0.0"
  },
...
app.post('/api/reviews',
  ...
  async (req, res) => {
    try {
      ...
      if (existing) return res.status(409).json({ success: false, message: 'Review already exists for this booking' });
      ...
      await booking.service.update({ avgRating, reviewCount: allReviews.length });
...
[deploy]
startCommand = "node -e \"const fs=require('fs'),path=require('path');const config=JSON.parse(fs.readFileSync('config/config.json','utf8'));config.production={use_env_variable:'DATABASE_URL',dialect:'postgres'};fs.writeFileSync('config/config.json',JSON.stringify(config,null,2));console.log('✅ Database config fixed!');\" && npm run migrate && npm start"
...
# ─── Database ─────────────────────────────────────
# PostgreSQL connection string from Railway dashboard
DATABASE_URL=postgresql://user:password@host:5432/onpurpose

# ─── Auth ─────────────────────────────────────────
# Generate with: node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"
JWT_SECRET=REPLACE_WITH_64_CHAR_HEX
REFRESH_TOKEN_SECRET=REPLACE_WITH_64_CHAR_HEX
JWT_EXPIRE=7d

# ─── Security ─────────────────────────────────────
BCRYPT_PEPPER=REPLACE_WITH_32_CHAR_HEX
QR_SECRET=REPLACE_WITH_32_CHAR_HEX

# ─── CORS ─────────────────────────────────────────
CORS_ORIGINS=https://onpurpose.earth,https://www.onpurpose.earth
CORS_ORIGIN=https://onpurpose.earth
...
### User Model
- UUID primary key
- Fields: name, email, password, role, isVerified, isSuspended, bio, location, avatar
- Authentication fields: verifyToken, resetToken, refreshTokenHash
- Integration fields: stripeAccountId, stripeCustomerId
- Trust system: trustScore, verifiedCredential

### Service Model  
- UUID primary key
- Fields: title, description, price, category, providerId
- Availability: duration, location, isOnline
- Rating system: avgRating, reviewCount
- Status: isActive

### Booking Model
- UUID primary key
- Core: userId, serviceId, date, time, status
- Financial: totalAmount, platformFee, providerAmount, paymentStatus
- Stripe integration: stripeSessionId, stripePaymentIntentId
- Session tracking: qrTokenHash, sessionStartTime, sessionEndTime
- Escrow system: escrowReleaseScheduled, escrowReleaseAt
- Recurring: isRecurring, recurringGroupId

### Review Model
- UUID primary key
- Links: bookingId, userId, serviceId
- Content: rating (1-5), comment
- Unique constraint on bookingId

### Availability Model
- UUID primary key
- Provider scheduling: providerId, dayOfWeek, startTime, endTime

### BlockedDate Model
- UUID primary key
- Provider availability: providerId, date, reason

### EarlyAccess Model
- UUID primary key
- Email capture: email (unique)
...
### Services
- GET /api/services - List services (with search, filters)
- POST /api/services - Create service (provider/admin)
- GET /api/services/my-services - My services (provider/admin)
- GET /api/services/:id - Get service details
- PATCH /api/services/:id - Update service (provider/admin)
...
- Proper startup sequence with migrations

## Current Issues

1. **Railway Deployment**: Database migration failing during build phase
2. **GitHub Push**: Blocked by secret detection in documentation files
3. **Environment Variables**: Need proper JWT_SECRET and DATABASE_URL configuration
...
This complete codebase provides ChatGPT with full context of the OnPurpose marketplace application.
