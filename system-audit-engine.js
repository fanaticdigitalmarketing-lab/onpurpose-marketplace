// ONPURPOSE SYSTEM AUDIT ENGINE
// Comprehensive audit, auto-fix, and validation system
...
class SystemAuditEngine {
  ...
  async performGlobalAnalysis() {
    ...
    console.log(`📊 Analyzed ${this.stats.totalFiles} files`);
  ...
  getAllFiles() {
    const files = [];
    const extensions = ['.js', '.html', '.json', '.md'];
  ...
  async analyzeJavaScriptFile(filePath, content) {
    ...
    if (filePath.includes('server.js') || filePath.includes('server')) {
      ...
      for (const route of allAsyncRoutes) {
        if (!content.includes(route.replace(/\).*$/, ') { try {'))) {
          this.issues.push({
    ...
    if (content.includes('.then(') && !content.includes('.catch(')) {
      const thenChains = content.match(/\.then\s*\([^)]+\)/g) || [];
    ...
    if (filePath.includes('server.js') || filePath.includes('api')) {
      const routes = content.match(/app\.(get|post|put|delete)/g) || [];
      for (const route of routes) {
        if (!content.includes('express-validator') && !content.includes('req.body') && !content.includes('req.params')) {
          continue;
        }
        
        // Check if route has validation
        const routeIndex = content.indexOf(route);
        const routeBlock = content.substring(routeIndex, routeIndex + 500);
        
        if (routeBlock.includes('req.body') && !routeBlock.includes('validation') && !routeBlock.includes('check(')) {
          this.issues.push({
  ...
  addTryCatch(content, issue) {
    // Add try-catch wrapper to async routes
    const routePattern = /(app\.(get|post|put|delete)\s*\([^)]+\)\s*[,)]\s*async\s*\([^)]*\)\s*=>\s*{)/g;
    
    return content.replace(routePattern, (match) => {
  ...
  async executeFullAudit() {
    console.log('🔍 ONPURPOSE SYSTEM AUDIT ENGINE');
    console.log('='.repeat(60));
...
// Execute the audit
const auditEngine = new SystemAuditEngine();
